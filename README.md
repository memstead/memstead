...
The two products are also on crates.io, so `cargo install memstead-cli` and `cargo install memstead-mcp` work: [`memstead-cli`](https://crates.io/crates/memstead-cli) and [`memstead-mcp`](https://crates.io/crates/memstead-mcp). Three more crates are published only because they are their dependencies, with no API promise: [`memstead-base`](https://crates.io/crates/memstead-base) (engine internals), [`memstead-schema`](https://crates.io/crates/memstead-schema) and [`memstead-git-branch`](https://crates.io/crates/memstead-git-branch) (the git-backed storage backend). They ride the engine's version line, so a set pinned to one version works together, but they are pre-1.0 and change without deprecation cycles whenever the products need it. The binaries above stay the supported way to *install* Memstead.
...
- **Claude Code:** install the [plugin](plugins/claude-code/) and run its `/setup` skill, which resolves the binary path, initialises the workspace, writes `.mcp.json`, and tells you to restart. This is the paved path:
