...
- **The WASM reference states archive compatibility**, generated from
  the engine's own format constants: which format the CLI writes, which
  formats the published package reads, and the versions of each. Neither
  crates.io nor npm carried that fact, so the only way to find out was
  to install both and write a client against whatever came out.
