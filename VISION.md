...
Memstead does **not** claim that an agent reads, understands, or answers better because the knowledge is typed. The project's own controlled evaluation ([docs/proof/substrate/](docs/proof/substrate/)) put a schema-forced typed mem against equally-curated free-form notes and measured a signed answer-quality delta of ≈ −0.010 ± 0.006 — no measurable read-side advantage. The token saving that does show up traces to *curation*, which flat notes share, not to typing. Two independent measurements close that axis, and both are narrow. This one is in-window: the whole corpus fits the budget, so it measures answer-time structuring and nothing else. The second is off-window at roughly 313k tokens, where a flat markdown corpus reached a fixed audit-recall target in five runs of five and the typed workspace in three of five, at about 1.5 times the cost; it was replicated at n=3 per arm against a pre-registered verdict rule, every credited detection was human-verified, and the typed arm demonstrably used the engine's query surface rather than reading files around it. One task each, one corpus each, one model each. **They close the read side as measured, not as a matter of principle**, and a measurement that reopened it would be welcome rather than unwelcome.

That result is scoped to the read side — answer quality over a finished, equally-curated holding. It says nothing about the write side, where the claims below live: whether knowledge gets created completely and consistently in the first place, stays coherent as many actors write to it, and transfers with its provenance intact. Read carefully, the evaluation *locates* the value rather than denying it: curation is what pays — and curation is precisely what this engine mechanizes. Schemas force the questions a subject must answer, write-time validation refuses what does not conform, and the result is **agent-curated knowledge**: deliberately selected, structured, and maintained, not extracted. Citing the read-side null against these write-side mechanisms is citing it out of scope.
...
Memstead's category is agent memory: the systems that give AI agents durable state — mem0, Zep/Graphiti, Letta, basic-memory, IWE, and the memory built into coding agents themselves. Most of the category *derives* memory: an extraction pipeline distils conversations into a vector or graph store and optimises recall. Memstead sits at the *authored* end: agents and humans deliberately write typed entities into a schema-validated graph, stored as markdown in a git repository the user owns. Derived memory optimises what an agent recalls; an authored mem is an agent-curated model of a subject — reviewable in diffs, portable as a versioned package, and correct by construction rather than by extraction. [PRIOR_ART.md](PRIOR_ART.md#the-2026-agent-memory-category) carries the tool-by-tool comparison.
