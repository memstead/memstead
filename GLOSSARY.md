...
The source `preparation` slot names an engine-registered preparation — the graded design (2026-08-22) gives it three flavours on text and graph mediums: content preparation (an entity's schema-declared load-bearing sections as its prepared form), delivery preparation (sub-file units in a deterministic total order), and codebase→code-map. PDF/DOCX/audio conversion is an explicit non-goal (agent-side extraction plus the raw-byte hash fallback suffice). No preparation implementation ships today. A source that declares a preparation step is **skipped at run time**, not refused at declaration: `projection init` has no way to declare one, and a record that acquires one another way (a hand edit, a migration) is accepted at rest — the run that would consume it reports `Ingest "<binding>" is unsupported: facet "<name>" declares preparation "<step>", which has no implementation. Skipping.` and exits 0. Nothing silently half-runs — but nothing warns you at the moment you declare it either.
