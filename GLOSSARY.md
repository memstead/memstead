...
The source `preparation` slot names an engine-registered preparation: the graded design (2026-08-22) gives it three flavours on text and graph mediums, content preparation (an entity's schema-declared load-bearing sections as its prepared form), delivery preparation (sub-file units in a deterministic total order), and codebase→code-map. PDF/DOCX/audio conversion is an explicit non-goal (agent-side extraction plus the raw-byte hash fallback suffice). The engine owns the **preparation registry** and consults it at two touchpoints: anchor observation asks it for an artifact's prepared form before hashing (the binding-backed verify and the standalone `verify-anchors` share that one site, so both inherit every entry), and ingest delivery will ask it for a source's unit sequence (reserved). One flavour ships today, `entity-load-bearing` over graph sources: an entity-grain anchor hashes the stable serialization of its type's load-bearing sections (a section's `load_bearing` flag, else the required sections, else every section), so a notes-only edit keeps a dependent's anchor resolving while a load-bearing edit drifts it. The `url` grain acquires its prepared form the same way path grains do, over the content its observer supplies at write time (the anchor's `content`), and defaults to `hash_stability: unstable`. Every edit path refuses an identifier the registry does not know (`PreparationUnsupported`, naming the registered set); a record that acquired one another way (a hand edit) is accepted at rest and refused when a run would consume it: the brief reports `Ingest "<binding>" is unsupported: source "<name>" declares preparation "<step>", which is not in this engine's preparation registry (registered: …). Skipping.` and exits 0. Nothing silently half-runs. `PREPARATION_IMPL_VERSION` (1 since the first flavour) is hashed into every binding's `hash(D)`, so landing or changing an implementation supersedes every prior finding by construction.
