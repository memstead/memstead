...
The definition separates four things the codebase historically conflated: the *logical collection* (mem), the *storage form* (folder / branch / archive), the *modal flavour* (knowledge / planning / spec / …), and the *mount granularity* (a mem-repo gitdir contains N mems).
...
**Examples.** The Memstead project's own mem-repo carries three mems with the `software` schema; each has a different subject — *the engine codebase*, *the Claude Code plugin*, *the registry server* — while *the project as a whole* is a fourth mem on its own `project` schema. Same software schema, three distinct mems, because three distinct subjects.
...
- A flavour is a *read-back* of the schema choice, not an independent axis. `default` is hybrid because of the types it declares; `planning` is the planning flavour for the same reason.
...
- The **facet half** — *which part of it, engaged how*: a scope (allow / deny patterns), an optional engagement contract (verbs, tools, discipline), and an optional preparation step (content preparation over load-bearing sections, delivery unitization/ordering, codebase→code-map).
...
**Build, then measure and maintain.** Populating a mem is only the first half. Once a binding has built content, the engine can **verify** it — deterministically measure how faithfully the destination mem still reflects its sources (grain-classed coverage, anchor resolution, freshness), recording durable findings into an engine-owned store keyed to the binding declaration and the source head. Verify mutates no destination entity, but it is not a pure read: besides the findings store it backfills observed content hashes onto hash-less anchors, which is measurement machinery. Its one write into the mem's own config, the `#verified` freshness baseline, rides `projection verify --advance`, so a gate or a grader that verifies in order to read leaves that config byte-identical. **Sync** is the single maintenance writer — it consumes those findings and the changed-source slice through one brief an agent acts on. **Prune** proposes removing destination entities whose source is gone, presenting both sides and never overwriting a model-side edit. Every repair reaches the mem through the normal mutation surface, driven by the sync brief, not by the measurement path.

**History.** An earlier single `Scope` primitive conflated territory-selection and engagement. A four-primitive model then split territory (**Medium**), engagement (**Facet**), and obligation (Projection), with a separate **Ingest** run-config carrying a write-mode (`discovery` / `refinement` / `one-shot`), trigger, and batch size. The ingest was folded into the binding first (detached run-configs drift; `refinement`-as-writer is replaced by verify + sync). The 2026-07 consolidation then folded the standalone medium and facet records in too — the same reasons applied verbatim: the per-destination-mem store namespacing already capped their reuse at intra-mem sharing that never occurred (every live pipeline was 1:1:1), while every consumer paid for three files, cross-record reference integrity, create-ordering, and orphan cleanup. The engine reads only the single-record v2 format, with no compatibility layer: a file in a prior format is quarantined as retired and re-authored with `memstead projection init`. (The `/ingest` plugin *skill* is unrelated to the retired run-config and stays current — it is the agent-side workflow that drives a binding's `build` operation.)

The source `preparation` slot names an engine-registered preparation: the graded design (2026-08-22) gives it three flavours on text and graph mediums, content preparation (an entity's schema-declared load-bearing sections as its prepared form), delivery preparation (sub-file units in a deterministic total order), and codebase→code-map. PDF/DOCX/audio conversion is an explicit non-goal (agent-side extraction plus the raw-byte hash fallback suffice). The engine owns the **preparation registry** and consults it at two touchpoints: anchor observation asks it for an artifact's prepared form before hashing (the binding-backed verify and the standalone `verify-anchors` share that one site, so both inherit every entry), and ingest delivery asks it for a source's unit sequence (one file, many units, one total order). Three flavours ship today. `entity-load-bearing` over graph sources (touchpoint A): an entity-grain anchor hashes the stable serialization of its type's load-bearing sections (a section's `load_bearing` flag, else the required sections, else every section), so a notes-only edit keeps a dependent's anchor resolving while a load-bearing edit drifts it. `dated-entries` over path-shaped sources (touchpoint B): a file is a sequence of entries opening with an ISO date or date-time, each a delivery unit `<path>#<stamp>`, and the source's units deliver in stamp order across files, identical on every pass; a first run delivers every unit, a change run only the units that changed, at their ordered positions, and a span anchor over a unit hashes the unit rather than its file. `code-map` over path-shaped sources (touchpoint A): a scoped file's prepared form is its interface digest (imports, exports, declarations and signatures; comments, formatting and bodies invisible; heuristic, by language family), and a tree anchor's is the code map of every scoped file under it, so an anchor on code drifts when an interface changes and stays quiet when only an implementation does. The `tree` grain has a prepared form only under `code-map`; elsewhere a tree anchor is recorded but unhashed and resolves `recheck` (the stated remainder). The `url` grain acquires its prepared form the same way path grains do, over the content its observer supplies at write time (the anchor's `content`), and defaults to `hash_stability: unstable`; set `hash_stability: stable` on an immutable document so a changed hash reads as drift. The engine never fetches: a url row is re-adjudicated from observer-supplied observations (`memstead verify-anchors --observations`), each recorded on the row as `last_observed` (anchors sidecar version 2) so every anchor surface can say how long the row has gone unobserved. Every edit path refuses an identifier the registry does not know (`PreparationUnsupported`, naming the registered set); a record that acquired one another way (a hand edit) is accepted at rest and refused when a run would consume it: the brief reports `Ingest "<binding>" is unsupported: source "<name>" declares preparation "<step>", which is not in this engine's preparation registry (registered: …). Skipping.` and exits 0. Nothing silently half-runs. `PREPARATION_IMPL_VERSION` (bumped once per landed implementation; 3 since the code-map flavour) is hashed into every binding's `hash(D)`, so landing or changing an implementation supersedes every prior finding by construction.
