...
namespace memstead {
    ...
    // Render the due-brief for the workspace at `workspace_root` —
    // open entities whose schema-declared due date falls inside the
    // window (`within`: 90d/6m/2y forms; null applies the engine
    // default), overdue first. Byte-identical to `memstead due`: both
    // call the one shared engine entry point. `today` (YYYY-MM-DD)
    // overrides the current date for deterministic rendering.
    [Throws=MemsteadError]
    string due_brief(string workspace_root, string? within, string? mem, string? today);
};
```
...
interface Engine {
    ...
    // Pipeline edits — the v2 single-record model. The macOS pipeline editor
    // routes binding edits here instead of hand-writing `.memstead/` JSON.
    // A pipeline is ONE record: the binding, carrying its sources inline
    // (each source holds what the retired standalone medium + facet records
    // held). The eight medium/facet edit methods are gone with those record
    // kinds (2026-07 consolidation). See `memstead_base::pipeline_edit`.
    //
    // `add_projection` / `update_projection` carry a JSON **patch over the
    // full author-editable binding record** — intent / sources /
    // reference_mems / destination_mem / deny_paths / coverage_semantics /
    // rules / prune / the whole `operations` block. Patch semantics: an
    // absent field is preserved from the stored record; explicit `null`
    // clears intent / rules / prune; a present `sources` or `operations`
    // block replaces that block as a unit. `version` is engine-managed
    // (ignored in the payload, preserved on update), and unknown additive
    // keys are tolerated. Candidate records are validated in-record (source
    // name rules + the medium-capability matrix) before anything is written;
    // only refusals the edit *introduces* block it (typed ValidationFailed,
    // message + remedy verbatim from the engine).
    //
    ...
    // The macOS pipeline editor deserializes this to display the store.
    // Shape: `{ bindings:[{mem,name,config}] }` — the v2 single-record
    // store; `config` carries the whole binding (inline `sources`,
    // `operations`, everything). The `mediums` / `facets` / `ingests` keys
    // are gone with their record kinds. Cannot fail.
