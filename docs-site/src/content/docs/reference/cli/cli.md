...
* `--mem <MEM>` — Scope the report to one writable mem (the engine still loads every mem: dangling-link adjudication and the community partition are only truthful over the whole store)
