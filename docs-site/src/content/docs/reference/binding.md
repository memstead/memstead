...
- **`graph` verify is inert today**, despite its *yes* in **Enumerable**: a binding whose source is another mem enumerates nothing and leaves its anchors permanently unobserved, so coverage reads `0/0`. The fidelity report refuses to call that clean (the rollup returns `inconclusive`), but the measurement is not there yet — do not gate on a graph-source binding. The columns describe the medium's declared capability, not the current state of every operation over it.
