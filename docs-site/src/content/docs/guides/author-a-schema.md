...
- **`transition_requires_checks`** — a write landing `field` at `to_value` requires every entity related via the named `relationships` in `direction` (`incoming` / `outgoing`) to carry a fresh confirming check record (derived verification state `checked_ok` — stale and failed do not confirm). The graded-completion gate as data: "a plan may become `complete` only when every criterion pointing VERIFIES at it has been checked". Defaults to `block`; the refusal (and the health `constraints` finding, when a check goes stale after the transition) lists each unconfirmed entity with the state it derived. An empty related set satisfies — pair with `required_outgoing` where at least one related entity must exist. Verdicts ride the check ledger (`memstead check`), never an entity field, so the gate cannot be satisfied by editing the entity. An optional `min_related` (default 0) sets a floor on how many related entities the transition needs: `min_related: 1` refuses the vacuous case, an entity nothing points at landing the gated value with nothing to check.
- **`transition_requires_self_check`** — a write landing `field` at `to_value` requires the entity ITSELF to carry a fresh confirming check record of `check_kind` (`verification`, `conformance`, or a foreign `x-<name>` kind the engine records verbatim), recorded under an identity other than the entity's own author. The closing-contract gate as data: "a bundle may become `complete` only once someone else has recorded that its decisions were projected". The engine requires the recorded act and never judges what it asserts. Defaults to `block`; the refusal names the kind and the standing the entity derived instead (`never_checked`, `check_stale`, `self_checked`, ...). A malformed `check_kind` refuses at `schema validate`.
