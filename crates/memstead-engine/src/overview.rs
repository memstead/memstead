...
pub struct OverviewOutput {
    pub markdown: String,
    pub warnings: Vec<memstead_base::WarningHint>,
...
pub fn compose_overview(
    ...
    // --- include validation ---
    let mut warnings: Vec<memstead_base::WarningHint> = Vec::new();
    ...
    for cid in &cluster_ids {
        ...
        let summary =
            memstead_base::graph::community::generate_auto_summary(engine.store(), &info.entities);
    ...
    for cr in &create_rules {
        ...
        for raw in &cr.schemas {
            let canon: String = if raw == memstead_base::SCHEMA_WILDCARD {
                "*".to_string()
            } else {
