//! Integration tests for the pro vault-lifecycle orchestrators
//! (`create_vault`, `delete_vault`). These tests construct an
...
use memstead_base::storage::FilesystemVaultWriter;
...
use memstead_engine::{ProEngineError, vault_management};
...
fn folder_mount(vault: &str, path: PathBuf) -> Mount {
    Mount {
...
#[test]
fn create_vault_rejects_overlong_note() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("seed");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    let mut engine = Engine::from_mounts(vec![(
        folder_mount("seed", vault_dir),
        Box::new(writer) as Box<dyn VaultBackend>,
    )])
    .unwrap();

    let err = vault_management::create_vault(
        &mut engine,
        vault_management::VaultCreateParams {
            write_guidance: Default::default(),
            name: "alpha".to_string(),
            vcs: None,
            location: tmp.path().join("alpha"),
            schema_ref: "default@1.0.0".parse().unwrap(),
            note: Some("x".repeat(vault_management::NOTE_MAX_LEN + 1)),
            operator_mode: false,
        recovery: None,
        },
    )
...
#[test]
fn create_vault_rejects_when_no_allowlist_configured() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("seed");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    .unwrap_err();
    match err {
        ProEngineError::VaultPathNotAllowed { reason, .. } => {
            assert_eq!(reason, "no_allowlist_configured");
        }
        other => panic!("expected VaultPathNotAllowed, got {other:?}"),
    }
}

#[test]
fn create_vault_rejects_name_collision() {
    // Engine boots with vault "alpha"; trying to create another
    // "alpha" surfaces VaultNameCollision before any disk write.
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("alpha");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    let mut engine = Engine::from_mounts(vec![(
        folder_mount("alpha", vault_dir),
        Box::new(writer) as Box<dyn VaultBackend>,
    )])
    .unwrap();
    // Wildcard allowlist so the allowlist check doesn't bounce us first.
    engine.set_settings(WorkspaceSettings {
        vault_create_rules: vec![CreateRuleSetting {
            pattern: "*".to_string(),
    ...
    .unwrap_err();
    match err {
        ProEngineError::Basis(EngineError::VaultNameCollision { name, .. }) => {
...
#[test]
fn create_vault_succeeds_with_wildcard_rule() {
    // Wildcard allowlist + matching basename → vault is created,
    ...
    let response = vault_management::create_vault(
        &mut engine,
        vault_management::VaultCreateParams {
            write_guidance: Default::default(),
            name: "alpha".to_string(),
...
/// candidate IS the vault name.
#[test]
fn create_vault_with_hierarchical_name_matches_path_rule() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("seed");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    assert!(engine.vault_router().is_writable("planning/alpha"));
    assert!(!engine.vault_router().is_writable("alpha"));
}

/// The vault-name grammar refuses leading-underscore segments
/// (reserved for registry refs like `__MEMSTEAD`). Validation fires
/// before any disk side effect.
#[test]
fn create_vault_rejects_double_underscore_segment() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("seed");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    match err {
        ...
        // `InvalidVaultName` variant with the `reserved_prefix` reason
...
fn create_vault_structural_invalid_name_matrix() {
    ...
    for (name, expected_reason) in cases {
        ...
        .unwrap_err();
        match err {
            ProEngineError::InvalidVaultName { name: got_name, reason } => {
            ...
            other => panic!(
                "input {name:?} expected InvalidVaultName (reason={expected_reason}), got {other:?}"
            ),
        }
    }
}
...
#[test]
fn create_vault_rejects_basename_mismatch() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("seed");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
...
#[test]
fn delete_vault_rejects_when_no_allowlist_configured() {
    // Engine with no [[vault_management.delete]] rules → V1
    // unified always errors with VaultPathNotAllowed.
    ...
    let err = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
    ...
    .unwrap_err();
    match err {
        ProEngineError::VaultPathNotAllowed { reason, candidate, .. } => {
            assert_eq!(reason, "no_allowlist_configured");
            assert_eq!(candidate, "specs");
        }
        other => panic!("expected VaultPathNotAllowed, got {other:?}"),
    }
    // Engine state unchanged — vault still writable.
    assert!(engine.vault_router().is_writable("specs"));
}

#[test]
fn delete_vault_rejects_unknown_name() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("specs");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    assert!(matches!(
        err,
        ProEngineError::Basis(EngineError::UnknownVault(v)) if v == "missing"
    ));
}

#[test]
fn delete_vault_rejects_overlong_note() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("specs");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    let long_note = "x".repeat(vault_management::NOTE_MAX_LEN + 1);
    let err = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
            name: "specs".to_string(),
...
#[test]
fn delete_vault_unregisters_when_allowlist_matches() {
    // Build an engine with a delete-rule that admits the vault.
    // Call delete_vault; expect the vault unregistered + files
    ...
    engine.set_settings(WorkspaceSettings {
        ...
        vault_delete_rules: vec![DeleteRuleSetting {
    ...
    let response = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
            name: "specs".to_string(),
            delete_files: false,
    ...
    // Engine state: vault unregistered, directory still on disk.
    assert!(!engine.vault_router().is_writable("specs"));
    assert!(vault_dir.exists(), "delete_files=false leaves directory");
}
...
/// `vault_path` (None for flat) and composes the same bare-name
...
fn create_delete_round_trip_flat_namespace() {
    ...
    .unwrap();
    assert!(engine.vault_router().is_writable("exec-foo"));
...
fn create_delete_round_trip_hierarchical_namespace() {
    ...
    assert!(engine.vault_router().is_writable("planning/plan-foo"));
    assert!(!engine.vault_router().is_writable("plan-foo"));
...
/// the hierarchical pattern surfaces `VAULT_PATH_NOT_ALLOWED` with the
/// full vault name as the candidate. Hierarchical paths are the
/// identifier —
/// the agent reads `planning/plan-foo` directly without any
/// composition step.
#[test]
fn delete_vault_hierarchical_name_in_path_not_allowed_envelope() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("seed");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    match err {
        ProEngineError::VaultPathNotAllowed { reason, candidate, .. } => {
            ...
            // The candidate IS the vault name verbatim — no
            // composition step.
...
#[test]
fn delete_vault_with_delete_files_removes_directory() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("specs");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    assert!(response.deleted_from_router);
    assert!(response.files_deleted);
    assert!(!vault_dir.exists(), "delete_files=true removes directory");
}

// ---------------------------------------------------------------------------
// Item 01 — operator-mode bypass for vault-lifecycle gates
// ---------------------------------------------------------------------------

/// `operator_mode = true` on `create_vault` admits a path/name
/// combination against a workspace whose `[[vault_management.create]]`
/// list is empty. The same call with `operator_mode = false` returns
/// `VAULT_PATH_NOT_ALLOWED` reason=`no_allowlist_configured` —
/// confirming the bypass is scoped to operator-mode and agent-mode
/// behaviour is unchanged.
#[test]
fn create_vault_operator_mode_bypasses_empty_allowlist() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("seed");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    let agent_err = vault_management::create_vault(
        &mut engine,
        vault_management::VaultCreateParams {
            write_guidance: Default::default(),
            name: "alpha".to_string(),
...
#[test]
fn create_vault_operator_mode_still_enforces_input_validation() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("seed");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
...
/// `operator_mode = true` on `delete_vault` admits an unregister
/// against a workspace whose `[[vault_management.delete]]` list is
/// empty. Agent-mode against the same workspace returns
/// `VAULT_PATH_NOT_ALLOWED` reason=`no_allowlist_configured`.
#[test]
fn delete_vault_operator_mode_bypasses_empty_allowlist() {
    let tmp = TempDir::new().unwrap();
    let vault_dir = tmp.path().join("specs");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    ...
    // Agent-mode rejects.
    let agent_err = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
            name: "specs".to_string(),
            delete_files: false,
...
/// The `VAULT_REFERENCED_BY_POLICY` check gates on `delete_files: true`
...
/// data.
#[test]
fn delete_vault_policy_check_gates_on_delete_files() {
    ...
    let target_writer = FilesystemVaultWriter::new(target_dir.clone());
    let referrer_writer = FilesystemVaultWriter::new(referrer_dir.clone());
    let mut engine = Engine::from_mounts(vec![
        (
            folder_mount("target", target_dir.clone()),
            Box::new(target_writer) as Box<dyn VaultBackend>,
        ),
        (
            folder_mount("referrer", referrer_dir),
            Box::new(referrer_writer) as Box<dyn VaultBackend>,
        ),
    ])
    .unwrap();
    ...
    let agent_destroy_err = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
    ...
    match agent_destroy_err {
        ProEngineError::VaultReferencedByPolicy { name, referring_vaults } => {
            assert_eq!(name, "target");
            assert_eq!(referring_vaults, vec!["referrer".to_string()]);
        }
        other => panic!(
            "expected VaultReferencedByPolicy for agent-mode delete_files=true, got {other:?}"
        ),
    }
    ...
    let operator_destroy_err = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
            name: "target".to_string(),
            delete_files: true,
    ...
    match operator_destroy_err {
        ProEngineError::VaultReferencedByPolicy { name, .. } => {
            assert_eq!(name, "target");
        }
        other => panic!(
            "expected VaultReferencedByPolicy for operator-mode delete_files=true, got {other:?}"
        ),
    }

    // Agent-mode + delete_files: false → ADMITTED. The grant
    // survives against the preserved storage.
    let agent_unregister = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
            name: "target".to_string(),
            delete_files: false,
    ...
    assert!(!engine.vault_router().is_writable("target"));
}

// ---------------------------------------------------------------------------
// Vault-delete referrer scan. Revoking a grant closes the workspace-
// policy axis but not the edge-graph axis. Without this check, a vault
// whose grant was revoked but whose surviving Write-Vault peers still
// carry cross-vault edges into it deletes cleanly, leaving dangling
// references that resolve to nothing.
// ---------------------------------------------------------------------------

/// Positive complement. A delete where the doomed vault has cross-vault
/// Write-Vault incoming edges refuses with the typed
/// `VAULT_HAS_INCOMING_REFS` envelope listing every offending source.
#[test]
fn delete_vault_refuses_when_cross_vault_incoming_edges_remain() {
    ...
    let tmp = TempDir::new().unwrap();
    let a_dir = tmp.path().join("vault-a");
    let b_dir = tmp.path().join("vault-b");
    ...
    .unwrap();
    // Allow `a → b` cross-vault links so the relate call below
    // succeeds. The grant is revoked before the delete attempt so
    // the policy axis (Step 3, `VAULT_REFERENCED_BY_POLICY`) passes
    ...
    // Revoke the cross-vault grant — Step 3 now passes — leaving
    ...
    // Vault-delete refuses with the typed envelope.
    let err = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
            name: "b".to_string(),
    ...
    .unwrap_err();
    match err {
        ProEngineError::Basis(EngineError::VaultHasIncomingRefs { vault, referrers }) => {
            assert_eq!(vault, "b");
            assert_eq!(referrers.len(), 1, "exactly one Write-Vault referrer");
            assert_eq!(referrers[0].from_id, "a--source");
            assert_eq!(referrers[0].vault, "a");
            assert_eq!(referrers[0].rel_types, vec!["DEPENDS_ON".to_string()]);
        }
        other => panic!("expected VaultHasIncomingRefs, got {other:?}"),
    }
    // Vault b still routable — refusal is pre-unregister.
    assert!(engine.vault_router().is_writable("b"));

    // Re-grant the cross-vault link so the relate --remove call
...
/// The `VAULT_HAS_INCOMING_REFS` gate fires unconditional — a router-only
/// unregister (`delete_files: false`) with stale cross-vault edges
...
/// unregister-only deletes against vaults whose surviving writable peers
/// still point at them are refused rather than silently admitted.
#[test]
fn delete_vault_router_only_refuses_when_cross_vault_incoming_edges_remain() {
    use memstead_base::ops::RelateArg;
    use memstead_base::vcs::Actor;
    ...
    // when the edge graph still carries a cross-vault reference into
    // the doomed vault. The policy axis (Step 3) is gated on
...
/// now-dangling `[cross_vault_links]` grants naming the deleted vault
/// — but PRESERVES the `[[vault_management.create]]` /
/// `[[vault_management.delete]]` allowlist rules, exact-name ones
...
fn destructive_delete_scrubs_cross_links_but_keeps_allowlist_rules() {
    ...
    // The doomed vault `other` grants `test` as a write target (key
    // match — scrubbed). A neutral `[cross_vault_links]` entry from
    // `test` grants `keep` as a value alongside the doomed vault to
    // verify the scrub drops only `other` and preserves the rest.
    // Pre-scrub the policy gate is empty because no surviving vault
    ...
    // policy gate passes regardless of the on-disk `[cross_vault_links]`
    ...
    // `[[vault_management.create]]` and `.delete]]` survive — deleting
    // the instance must not revoke the forward-looking permission to
    // re-create a vault of the same name.
    assert_eq!(
        after.matches("pattern = \"other\"").count(),
        2,
        "exact-name vault_management.{{create,delete}} rules for `other` must be preserved — got:\n{after}"
    );
    assert!(
    ...
    assert!(
        !engine.settings().cross_vault_links.contains_key("other"),
...
/// so a later reattach reactivates the cross-vault permissions.
#[test]
fn router_only_unregister_leaves_policy_intact() {
    ...
    let memstead = workspace.join(".memstead");
    std::fs::create_dir_all(&memstead).unwrap();
    let toml_body = "format = \"memstead-git-branch-1\"\n\n\
         [cross_vault_links]\n\
         other = [\"test\"]\n";
    ...
    let _ = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
            name: "other".to_string(),
            delete_files: false,
...
"#,
    ...
    let vault_dir = workspace.join("vault");
    std::fs::create_dir_all(&vault_dir).unwrap();
    let writer = FilesystemVaultWriter::new(vault_dir.clone());
    let mut engine = Engine::from_mounts(vec![(
        folder_mount("v", vault_dir),
        Box::new(writer) as Box<dyn VaultBackend>,
    )])
    .unwrap();
    ...
    assert!(
        engine.settings().cross_vault_links.contains_key("a"),
        "engine must surface the seed grant; got {:?}",
        engine.settings().cross_vault_links
    );
...
"#,
    ...
    assert!(
        engine.settings().cross_vault_links.is_empty(),
        "post-refresh cache must reflect the revoke; got {:?}",
        engine.settings().cross_vault_links
    );
}

/// Negative complement. A vault with no Write-Vault incoming edges
/// deletes cleanly; the edge-scan step doesn't fire false positives.
#[test]
fn delete_vault_succeeds_when_no_cross_vault_incoming_edges() {
    let tmp = TempDir::new().unwrap();
    let a_dir = tmp.path().join("vault-a");
    let b_dir = tmp.path().join("vault-b");
    std::fs::create_dir_all(&a_dir).unwrap();
    ...
    // No entities, no edges — pure metadata vault.
    let response = vault_management::delete_vault(
        &mut engine,
        vault_management::VaultDeleteParams {
            name: "b".to_string(),
...
/// `cross_vault_links_from_rules` workspace-policy posture, without being
/// materialized into `[cross_vault_links]`.
#[test]
fn rule_derived_cross_link_grant_is_enforced_and_surfaced_in_overview() {
    ...
    assert!(engine.cross_vault_link_allowed("scratch", "seed"));
    assert!(!engine.cross_vault_link_allowed("seed", "scratch"));
    assert!(!engine.cross_vault_link_allowed("scratch", "elsewhere"));

    // The explicit table is empty — nothing was materialized.
    assert!(engine.settings().cross_vault_links.is_empty());
    ...
    assert!(
        entries.iter().any(|(k, v)| *k == "cross_vault_links_from_rules" && v == "named"),
        "rule-derived grant must surface in the policy projection: {entries:?}",
    );
    assert!(
        !entries.iter().any(|(k, _)| *k == "cross_vault_links"),
        "explicit cross_vault_links must stay absent — nothing materialized: {entries:?}",
    );
    ...
    .unwrap();
    assert!(
        out.markdown.contains("Cross-vault links (rule-derived)")
    ...
    assert!(
        out.policy_flow
            .as_deref()
            .is_some_and(|f| f.contains("cross_vault_links_from_rules")),
...
fn explicit_and_rule_derived_cross_links_project_as_distinct_entries() {
    ...
    let entries = memstead_engine::overview::build_workspace_policy_entries(&engine);
    assert!(
        entries.iter().any(|(k, v)| *k == "cross_vault_links" && v == "wildcard"),
