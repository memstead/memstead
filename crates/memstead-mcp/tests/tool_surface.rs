...
fn response_shape_refs(tool_name: &str) -> &'static [&'static str] {
    match tool_name {
        ...
        "memstead_vault_set_version" => &[
            ...
            // `.mem` is the sealed-archive extension; `.mstd`/`.mdgv` are
            // its pre-rename spellings, still named in legacy-window prose.
            ".memstead", ".mdgv", ".mstd", ".mem", "config.json", "__MEMSTEAD", "vaults",
