...
pub struct HealthParams {
    ...
    #[schemars(
        description = "When true, the response carries the `[mutations]` posture (`mutations.require_notes`), the opaque `[plugin.*]` pass-through map, and a per-writable-mem `mems` detail array: `{ name, origin, storage, durable, vcs: { gitdir, head } }`, plus `title`, `subject`, `write_guidance` and `extra` where the mem's config sets them. Each `vcs` field is present where it exists: `gitdir` (the absolute canonical git directory of a mem-branch mount; a folder mem has none), `head` (the branch-tip SHA; omitted on a fresh mem with no commits yet), `worktree` (only where the mem has a working tree: a folder mem, or a mem-branch mount with an on-disk folder, never a plain mem-branch mount). For the sync flow and any tooling that needs the paths, so they never hardcode a layout or peel refs themselves. Documented alias: `include: [\"config\"]` renders the identical projection (the catalogue form every surface shares, including the CLI's `--include config`); passing both renders it once. Defaults to false — the absence of these fields is the default-posture signal. **Lifecycle policy** (`[[mem_management.create]]` / `[[mem_management.delete]]`) is surfaced via `memstead_overview`, not here — `memstead_health` is drift/diagnostics."
    )]
    #[serde(default)]
