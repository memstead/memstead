...
The graph's value comes from each type carrying what it was designed
for. A graph whose specs hold contracts, decisions or terms has lost the
structure the schema offers; a mem projected from code is mostly specs by
design, and the signal worth watching there is a cluster with no non-spec
entity beside its specs (the vital-signs health axis reads it from
`spec`'s `last_resort` declaration).
...
|---|---|---|
| `spec` | Internal component, module, or subsystem. The catch-all — never the default pick, but the home type for a surface in a code mem. Standing remarks (a frozen spec's historical-record marker) go in its Notes section, never in Identity. | Rule out contract, decision, requirement, principle, concept first. |
| `contract` | External wire-level surface (HTTP, gRPC, CLI, library API, file format), or an engine-owned durable state file (`protocol: engine_state`, its version constants listed in `version_axes` as `name=constant` pairs). | Is there a concrete request/response shape, or a format later engines must read back? |
...
| `requirement` | Normative MUST / SHOULD / MAY tied to one capability, testable, from a written source. Absent by design in a mem projected from code, which has no normative source. | Could a test verify compliance? |
