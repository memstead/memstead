...
impl Engine {
    ...
    /// [`Self::get_entity`] needs an id, and no other method produces
    /// one, so every caller had to pair the archive with an id list
    /// generated by the CLI — defeating the point of shipping a
    /// self-contained snapshot at all.
    ///
    ...
    #[wasm_bindgen(js_name = entityIds)]
    pub fn entity_ids(&self, mem: Option<String>) -> Result<JsValue, JsValue> {
        let mut ids: Vec<&str> = self
            .inner
            .store()
            .all_ids()
            .filter(|id| match mem.as_deref() {
                Some(want) => id.mem() == want,
                None => true,
            })
            .map(|id| id.as_ref())
            .collect();
        // Sorted so a caller rendering the list gets a stable order
        // across reloads of the same snapshot.
        ids.sort_unstable();
