...
//! gets a two-mount engine — their own ephemeral writable `sketch` vault plus
//! a shared read-only `memstead` content vault — behind a scoped remote-MCP
...
//! - `MEMSTEAD_SESSION_SCHEMA` — schema pin for the writable sketch vault
...
//!   override the curated read-only "what is Memstead" content vault source (a
//!   folder or a sealed `.mem`). When neither is set the server serves the
//!   curated content vault embedded in the binary, so the two-tier experience
//!   works out of the box.
//! - `MEMSTEAD_SESSION_CONTENT_SCHEMA` — schema pin for the content mount
//!   (default: the same pin as `MEMSTEAD_SESSION_SCHEMA`).
//! - `MEMSTEAD_SESSION_CONTENT_VAULT` — vault name the content mount registers
//!   under (default `memstead`). MUST match the content source's own vault name:
//!   an archive's ids are `<name>--slug`, so a mismatch makes its entities
//!   invisible. E.g. set `engine` to read a vault exported as `engine`.
...
use memstead_serve::session::{
    CONTENT_VAULT_NAME, DEFAULT_SESSION_ENTITY_CAP, DEFAULT_SESSION_MAX, SessionRegistry,
    build_sketch_app,
};
use uuid::Uuid;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // The sketch-vault pin for the connection-born `/mcp` flow (one engine per
    ...
    // The curated read-only content vault the agent reads to orient itself.
    // A folder or a sealed `.mem`; absent → an empty in-memory placeholder so
    // the two-mount structure holds until a curated vault is authored.
    let content_storage = if let Ok(dir) = std::env::var("MEMSTEAD_SESSION_CONTENT_DIR") {
        ...
        // No override → serve the embedded curated content vault. Falls back to
        ...
        match materialize_embedded_content() {
            Ok(dir) => {
                eprintln!(
                    "memstead-session-serve: serving the embedded curated content vault from {}",
                    dir.display()
                );
...
mounting an empty placeholder"
    ...
    // Vault name the read-only content mount registers under. Must equal the
    // content source's own vault name (an archive's ids are `<name>--slug`, so a
    // mismatch makes its entities invisible). Defaults to the curated content
    // vault's name; set it to e.g. `engine` to read a vault exported as `engine`.
    ...
    // release their vaults and their URLs start refusing (observable expiry).
    {
