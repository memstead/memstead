//! `memstead branch-reset <vault> <target_sha>` — the engine's history-
//! rewrite primitive exposed as a CLI subcommand.
//!
//! Replay workflows that need to rewind a vault's branch pointer to
...
/// `memstead branch-reset <vault> <target_sha>` arguments.
#[derive(Args, Debug)]
pub struct BranchResetArgs {
    /// Vault whose branch pointer to reset. Must be git-branch-backed.
    pub vault: String,
...
pub fn run(ctx: &CliContext, args: BranchResetArgs) -> anyhow::Result<()> {
    let outcome = match ctx.cli_engine()? {
        CliEngine::VaultRepo(mut engine) => engine
            .branch_reset(&args.vault, &args.target_sha)
        ...
        CliEngine::Filesystem(_) => {
            ...
            return Err(CliError {
                ...
                message: format!(
                    "vault '{}' is not git-backed — `memstead branch-reset` requires a git-branch mount",
                    args.vault,
                ),
                details: None,
            }
