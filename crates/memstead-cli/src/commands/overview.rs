...
/// warning when rich `--include` / `--vault` / `--token-budget` flags
...
pub struct Args {
    ...
    /// Scope schemas + vault inventory to a single writable vault.
    #[arg(long)]
    pub vault: Option<String>,

    /// Opt heavy content into the response: `community_members`,
    /// `community_bridges`, `vault_distribution`, `dangling_links`.
    ...
    /// `community_bridges`, `vault_distribution`, `dangling_links`).
    /// Hard-required content (vault roster, schema refs, community
...
pub fn run(ctx: &CliContext, args: Args) -> anyhow::Result<()> {
    // The full build always activates the vault-repo feature, so both
    ...
    let out = match compose_overview(&mut engine, composer_args, Surface::Cli) {
        ...
        Err(ComposeOverviewError::UnknownVault { name, writable_vaults }) => {
            return Err(CliError {
                code: "UNKNOWN_VAULT",
...
pub fn run(ctx: &CliContext, args: Args) -> anyhow::Result<()> {
    ...
    // `community_bridges`, `vault_distribution`, `dangling_links` need
    ...
    let pro_only_warning = (!args.include.is_empty()
        ...
        .then(|| {
            (
                ...
                "the lean build renders the simple cluster overview only — rich content (`--vault` scoping, `--include community_bridges` / `vault_distribution` / `dangling_links`, non-default `--token-budget`) requires the full `memstead` build or the `memstead_overview` MCP tool".to_string(),
            )
        });
...
mod tests {
    ...
    fn include_accepts_repeated_and_comma_split_forms() {
        ...
        assert_eq!(
            repeated.include,
            vec!["community_members", "vault_distribution"],
        );
