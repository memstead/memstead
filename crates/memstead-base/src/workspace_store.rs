...
//!   adapter declaration plus (in later sessions) cross-vault
...
//!   (per-mount vault name, schema pin, capability, lifecycle,
...
//! change often (planning vaults, ingest scratch vaults, …). Sharing
...
//! `Box<dyn VaultBackend>` is a separate concern — handled by
...
//! `vault-repo` Cargo feature; consumers in the basis flavour cannot
//! materialise a `MountStorage::GitBranch` mount and surface
//! [`InstantiateError::GitBranchRequiresVaultRepoFeature`].
...
use crate::storage::{ArchiveBackend, FilesystemVaultWriter, InMemoryBackend};
use crate::workspace::{
...
/// the per-vault meta directory
/// ([`memstead_schema::VAULT_META_DIR`], re-exported as
/// `crate::vault::VAULT_META_DIR`) and from the literal `".memstead/..."`
...
const WORKSPACE_TOML_FORMAT: &str = "memstead-git-branch-1";
/// Current `mounts.json` format. V2 stores `gitdir` / `path` values
/// relative to `workspace_root` when they live inside the workspace,
/// so checked-in state survives a clone into a different home dir.
/// Absolute paths are still accepted on write (and round-trip
/// untouched) when the mount target sits outside `workspace_root` —
/// e.g., an archive on a shared cache. The reader resolves relative
/// values against `workspace_root` at load time.
const MOUNTS_JSON_FORMAT_V2: &str = "memstead-mounts-2";
/// Legacy `mounts.json` format. Paths were stored as absolute
/// strings, baking the writing operator's home dir into committed
/// state and breaking portability. Reader keeps accepting V1 so
/// existing workspaces don't break; the first `save_state` rewrites
/// the file as V2, self-healing without an explicit migration step.
const MOUNTS_JSON_FORMAT_V1: &str = "memstead-mounts-1";
...
/// in a global cache, a vault on a separate filesystem — keep the
...
impl WorkspaceStoreAdapter for FileWorkspaceStore {
    fn load(&self, workspace_root: &Path) -> Result<Workspace, StoreError> {
        ...
        let mounts: Vec<Mount> = match std::fs::read_to_string(&mounts_path) {
            Ok(text) => {
                ...
                if doc.format != MOUNTS_JSON_FORMAT_V2
                    && doc.format != MOUNTS_JSON_FORMAT_V1
                {
                    return Err(StoreError::FormatMismatch {
                        path: mounts_path,
                        expected: MOUNTS_JSON_FORMAT_V2.to_string(),
                        found: doc.format,
                    });
                }
                doc.mounts
        ...
        let settings = build_settings(
            toml_doc.vault_management,
            toml_doc.cross_vault_links,
            toml_doc.mcp,
            toml_doc.mutations,
            toml_doc.plugin,
        )?;
    ...
    fn save_state(
        ...
        let doc = MountsJsonDoc {
            format: MOUNTS_JSON_FORMAT_V2.to_string(),
            mounts: workspace
                .mounts
                .iter()
...
/// writes. V1 carries the adapter declaration, `[vault_management]`
/// rule lists, and `[cross_vault_links]` permission policy. Plugin
...
struct WorkspaceTomlDoc {
    ...
    /// `[vault_management]` rule lists. Both arrays default to empty
    /// — an empty list means "no agent-driven vault create / delete
    /// allowed" (mirrors pro). Operators add `[[vault_management.create]]`
    /// / `[[vault_management.delete]]` entries to opt in.
    #[serde(default)]
    vault_management: VaultManagementWire,
    /// `[cross_vault_links]` raw shape — `<vault> = "*"` (wildcard)
    /// or `<vault> = ["target", ...]` (allowlist) per key. Parsed
...
/// Wire shape for one `[[vault_management.create]]` entry. Mirrors
...
/// top-level `[cross_vault_links]` section, sharing the wildcard-vs-
...
/// Wire shape for one `[[vault_management.delete]]` entry. Mirrors
...
/// the `[vault_management]` rules and the post-decoded
/// `[cross_vault_links]` map; surfaces a typed parse error if any
/// cross-link value violates the wildcard / list / non-empty
/// invariants.
fn build_settings(
    vm: VaultManagementWire,
    cross_vault_links_raw: toml::Table,
    mcp: McpSection,
    mutations: MutationsSection,
    plugin: std::collections::HashMap<String, toml::Table>,
    ...
    for r in vm.create {
        let default_cross_links = match r.default_cross_links {
            ...
            Some(value) => {
                let location = format!(
                    "[[vault_management.create]] pattern={}.default_cross_links",
                    r.pattern
                );
    ...
    let mut cross_vault_links = std::collections::BTreeMap::new();
    for (vault, value) in &cross_vault_links_raw {
        let location = format!("[cross_vault_links].{vault}");
        let parsed = parse_cross_link_value(&location, value)?;
        cross_vault_links.insert(vault.clone(), parsed);
    }

    Ok(WorkspaceSettings {
        vault_create_rules: create_rules,
        vault_delete_rules: vm
            .delete
            .into_iter()
...
struct MountWire {
    ...
    /// The authoritative pin is the vault's own `VaultConfig.schema`;
    ...
    /// settled vaults. `default` on read keeps pre-dual-pin
    /// `mounts.json` files loading unchanged; skip-on-`None` keeps
    /// settled vaults' entries byte-identical to before.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
enum MountStorageWire {
    ...
    /// vaults are normally constructed via `Engine::from_mounts`
...
pub enum InstantiateError {
    ...
    /// `vault-repo` Cargo feature). Pro consumers expose a
    /// feature-gated `instantiate_pro_backend` that handles all
    /// three variants.
    #[error(
        "vault {vault}: git-branch backend requires the `vault-repo` feature; \
         use `instantiate_pro_backend` from memstead-git-branch, or rebuild with --features vault-repo"
    )]
    GitBranchRequiresVaultRepoFeature { vault: String },
}

/// Materialise a [`VaultBackend`] for `mount` using the basis-flavour
...
pub fn instantiate_basis_backend(
    mount: &Mount,
) -> Result<Box<dyn VaultBackend>, InstantiateError> {
    match &mount.storage {
        MountStorage::Folder { path } => {
            Ok(Box::new(FilesystemVaultWriter::new(path.clone())))
        }
        ...
        MountStorage::GitBranch { .. } => {
            Err(InstantiateError::GitBranchRequiresVaultRepoFeature {
...
/// `memstead vault-repo init` to bootstrap a fresh workspace.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Layout {
    /// No `.memstead/workspace.toml` present — operator should run
    /// `memstead vault-repo init` to bootstrap.
    Empty,
...
/// vault — a directory carrying `.memstead/config.json` but **no**
/// `.memstead/workspace.toml`. The vault root *is* the workspace root
/// (the collapsed single-vault form), so the lone mount is a folder
/// backend pointed at `workspace_root` itself.
///
/// Returns `None` when the directory is not a standalone vault — no
...
/// collapses the old separate standalone-vault boot path into the unified
/// roster+detail experience: a lone vault opens as a workspace with one
/// mount, no `workspace.toml` required.
///
/// The synthesized workspace carries default (empty) settings — a
/// standalone vault has no `[vault_management]` / `[cross_vault_links]`
/// policy — and the mount is writable, not cross-linkable (there is no
/// sibling to link to). The schema pin and name come from the vault's own
...
mod tests {
    ...
    #[test]
    fn parse_workspace_settings_reflects_cross_vault_links_edit() {
        let tmp = TempDir::new().unwrap();
...
"#,
        ...
        assert!(
            settings.cross_vault_links.contains_key("team-a"),
            "initial parse must surface the team-a grant; got {:?}",
            settings.cross_vault_links
        );
...
"#,
        ...
        assert!(
            refreshed.cross_vault_links.is_empty(),
            "refreshed parse must drop the team-a grant; got {:?}",
            refreshed.cross_vault_links
        );
    }
...
"#,
        ...
        assert!(initial.vault_create_rules.is_empty());
...
"#,
        ...
        let refreshed = super::parse_workspace_settings(tmp.path()).unwrap();
        assert_eq!(refreshed.vault_create_rules.len(), 1);
        assert_eq!(refreshed.vault_create_rules[0].pattern, "test-*");
    }
...
"#,
    ...
    #[test]
    fn load_with_no_vault_management_yields_empty_settings() {
        // Workspace.toml without a `[vault_management]` section
        // produces the default empty settings — mirrors pro's
        // behaviour where missing rules mean "no agent-driven
        // vault create / delete allowed".
        let tmp = TempDir::new().unwrap();
        write_workspace_toml(
            tmp.path(),
            r#"
...
"#,
        ...
        let store = FileWorkspaceStore::new();
        let workspace = store.load(tmp.path()).unwrap();
        assert!(workspace.settings.vault_create_rules.is_empty());
        assert!(workspace.settings.vault_delete_rules.is_empty());
        assert!(workspace.settings.cross_vault_links.is_empty());
    }

    #[test]
    fn load_picks_up_cross_vault_links_wildcard_and_list() {
        // [cross_vault_links] is parsed via CrossLinkValue::parse_toml
        // to handle the wildcard ("*") vs allowlist ([...]) shape that
        // serde untagged-enum decode can't express. Both shapes round
        // through to WorkspaceSettings.cross_vault_links.
...
"#,
        ...
        let store = FileWorkspaceStore::new();
        let workspace = store.load(tmp.path()).unwrap();
        let cvl = &workspace.settings.cross_vault_links;
        assert_eq!(cvl.len(), 3);
    ...
    #[test]
    fn load_rejects_cross_vault_links_mixed_wildcard_and_names() {
...
"#,
        ...
        match err {
            StoreError::Parse { message, .. } => {
                assert!(message.contains("[cross_vault_links].specs"));
...
"#,
        ...
        let store = FileWorkspaceStore::new();
        let workspace = store.load(tmp.path()).unwrap();
        let rule = &workspace.settings.vault_create_rules[0];
        assert_eq!(rule.pattern, "exec-*");
        assert_eq!(rule.default_cross_links, Some(CrossLinkValue::Wildcard));
    }

    #[test]
    fn load_picks_up_vault_management_create_and_delete_rules() {
        // Operator-edited `[vault_management]` section flows through
...
"#,
        ...
        assert_eq!(workspace.settings.vault_create_rules[0].pattern, "exec-*");
        assert_eq!(
            workspace.settings.vault_create_rules[0].schemas,
            vec!["default@1.0.0".to_string(), "*".to_string()]
        );
        assert_eq!(workspace.settings.vault_create_rules[1].pattern, "scratch-*");
        assert_eq!(workspace.settings.vault_delete_rules.len(), 1);
        assert_eq!(workspace.settings.vault_delete_rules[0].pattern, "exec-*");
    }
    ...
    fn save_state_round_trips_migration_target() {
        ...
        let store = FileWorkspaceStore::new();
        let mut migrating = folder_mount("specs", PathBuf::from("/work/vault"));
...
"#,
        ...
        let original = Workspace {
            mounts: vec![
                folder_mount("specs", PathBuf::from("/work/vault")),
                Mount {
    ...
    fn save_state_round_trips_unset_schema_assertion() {
        ...
        // the authoritative pin lives in the vault's backend config, not
        // in the mount record.
        let tmp = TempDir::new().unwrap();
        write_workspace_toml(
            tmp.path(),
...
"#,
        ...
        let workspace = Workspace {
            mounts: vec![
                Mount {
                    ...
                    schema: Some(pin("default@1.0.0")),
                    storage: MountStorage::GitBranch {
                        gitdir: tmp.path().join("vault-repo").join(".git"),
                        branch: "engine".to_string(),
                    },
                    capability: MountCapability::Write,
                    lifecycle: MountLifecycle::Eager,
        ...
        let on_disk =
            std::fs::read_to_string(FileWorkspaceStore::mounts_json_path(tmp.path())).unwrap();
        // Format bumped to V2.
        assert!(on_disk.contains("\"memstead-mounts-2\""));
        // In-workspace path stored relative — no absolute prefix bake-in.
        assert!(
            on_disk.contains("\"vault-repo/.git\""),
        ...
        match &reloaded.mounts[0].storage {
            MountStorage::GitBranch { gitdir, .. } => {
                assert_eq!(gitdir, &tmp.path().join("vault-repo").join(".git"));
            }
    ...
    #[test]
    fn load_v1_with_absolute_paths_then_save_rewrites_as_v2_relative() {
        let tmp = TempDir::new().unwrap();
        write_workspace_toml(
            tmp.path(),
            r#"
...
"#,
        ...
        // Hand-write a legacy V1 mounts.json carrying an absolute
        // gitdir that lives inside this workspace_root — simulates
        // the "committed by another operator's home dir" failure
        // mode that motivated V2.
        let mounts_path = FileWorkspaceStore::mounts_json_path(tmp.path());
        std::fs::create_dir_all(mounts_path.parent().unwrap()).unwrap();
        let legacy_gitdir = tmp.path().join("vault-repo").join(".git");
  ...
  "mounts": [
    {{
      ...
      "cross_linkable": true
            ...
            legacy_gitdir.to_str().unwrap()
        );
        std::fs::write(&mounts_path, &legacy_body).unwrap();

        let store = FileWorkspaceStore::new();
        // V1 reader accepts the file; the absolute path round-trips
        // untouched (it's already absolute).
        let workspace = store.load(tmp.path()).unwrap();
        match &workspace.mounts[0].storage {
            MountStorage::GitBranch { gitdir, .. } => assert_eq!(gitdir, &legacy_gitdir),
            other => panic!("expected GitBranch storage, got {other:?}"),
        }

        // Saving the same workspace rewrites the file as V2 with a
        // relative path — self-healing migration, no explicit
        // command needed.
        store.save_state(tmp.path(), &workspace).unwrap();
        let on_disk = std::fs::read_to_string(&mounts_path).unwrap();
        assert!(on_disk.contains("\"memstead-mounts-2\""));
        assert!(on_disk.contains("\"vault-repo/.git\""));
...
"#,
        ...
        match err {
            StoreError::FormatMismatch {
                ...
                assert_eq!(expected, "memstead-git-branch-1");
...
"#,
    ...
    fn instantiate_basis_backend_handles_folder_archive_and_in_memory() {
        ...
        let _: Box<dyn VaultBackend> = instantiate_basis_backend(&folder).unwrap();
        let _: Box<dyn VaultBackend> = instantiate_basis_backend(&archive).unwrap();
        // The in-memory variant is a basis backend — no feature gate,
        // no path, materialises directly.
        let _: Box<dyn VaultBackend> = instantiate_basis_backend(&in_memory).unwrap();
    }
...
"#,
        ...
        let original = Workspace {
            mounts: vec![
                folder_mount("local", PathBuf::from("/work/vault")),
                Mount {
                    vault: "session".to_string(),
                    schema: Some(pin("default@1.0.0")),
                    storage: MountStorage::InMemory,
        ...
        let local = reloaded.mounts.iter().find(|m| m.vault == "local").unwrap();
    ...
    fn instantiate_basis_backend_rejects_git_branch_with_typed_error() {
        ...
        // `unwrap_err()` requires Box<dyn VaultBackend> to be Debug;
        ...
        match instantiate_basis_backend(&mount) {
            Err(InstantiateError::GitBranchRequiresVaultRepoFeature { vault }) => {
                assert_eq!(vault, "engine");
            }
            Ok(_) => panic!("expected GitBranchRequiresVaultRepoFeature, got Ok"),
        }
    }
