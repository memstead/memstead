//! filesystem-vault surfaces — single-vault, history-free, filesystem-backed
//! workspaces.
//!
//! Holds the helper modules consumed by the unified
//! [`crate::Engine`] when it routes through a folder-backed vault:
//! [`changelog`] (the JSONL provenance log), [`config`]
//! (`.memstead/config.json` reader / writer), [`publish`] (the
//! `.mem` archive assembler — engine-free), [`tier3`] (cross-vault
//! cache references).
//!
//! filesystem-vault `.memstead/config.json` is a different shape from the archive's
//! `.memstead/config.json` (which lives inside a published `.mem` zip):
//! the workspace shape carries cross-vault deps and other
