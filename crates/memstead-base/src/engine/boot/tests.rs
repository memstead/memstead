...
fn duplicate_mem_names_rejected_at_construction() {
    let tmp = TempDir::new().unwrap();
    let writer1: Box<dyn MemBackend> = Box::new(FilesystemBackend::new(tmp.path().to_path_buf()));
    let writer2: Box<dyn MemBackend> = Box::new(FilesystemBackend::new(tmp.path().to_path_buf()));
...
"#;
    ...
    let catalogue: Vec<std::sync::Arc<memstead_schema::Schema>> = ["0.9.0", "0.10.0", "0.2.0"]
        ...
        .collect();
    let newest =
        super::newest_builtin_version("gen-test", &catalogue).expect("name present in catalogue");
    ...
    let cycle_drops: Vec<_> = engine
        ...
        .filter_map(|w| match w {
            WarningHint::ParsedRelationInvalid {
                ...
                reason,
                ..
            } if reason == "cycle" => Some((entity_id.clone(), rel_type.clone(), target.clone())),
            _ => None,
        })
        .collect();
    assert_eq!(
...
fn load_on_init_collects_per_file_parse_errors_without_failing() {
    ...
    let mem_dir = tmp.path().to_path_buf();
    let writer = FilesystemBackend::new(mem_dir.clone());
    <FilesystemBackend as MemBackend>::write_entity(&writer, Path::new("good.md"), good.as_bytes())
        .unwrap();
    <FilesystemBackend as MemBackend>::write_entity(&writer, Path::new("bad.md"), bad.as_bytes())
        .unwrap();
    <FilesystemBackend as MemBackend>::commit(&writer, "seed", &CommitContext::internal()).unwrap();
...
fn from_workspace_root_loads_pipeline_configs_into_queryable_surface() {
    ...
    crate::pipeline_store::write_binding(tmp.path(), "specs", "graph", &sample_v2_binding("specs"))
        .unwrap();
...
fn sidecar_v1_fixture_loads_and_next_anchor_write_rewrites_v2_while_v3_refuses() {
    ...
    let on_disk = std::fs::read_to_string(mem_dir.join(".memstead").join("anchors.json")).unwrap();
...
fn state_of_report(report: &crate::engine::query::MemAnchorVerification, artifact: &str) -> String {
    report
        .anchors
        .iter()
        .find(|a| a.artifact == artifact)
...
"#;
    ...
    into_notes.sections = indexmap::IndexMap::from_iter([("body".to_string(), "body".to_string())]);
    engine
...
fn readonly_deferred_target_answers_from_storage() {
    ...
    let err = relate(&mut engine, "missing").expect_err("absent RO target keeps the typed refusal");
    assert!(
