//! In-memory [`VaultBackend`](crate::backend::VaultBackend) — the
//! writable, filesystem- and git-free companion to the folder backend.
//!
//! The vault lives entirely in RAM: committed entity bytes, the staged
//! mutation buffer, the provenance log, and the per-vault config all
//! sit on one [`Mutex`]-guarded [`State`]. Creating a backend
//! provisions nothing on disk; dropping it releases the vault with no
//! residue to clean up. Built to serve ephemeral per-session
//! playground vaults the session server spins up and tears down on
//! demand.
//!
//! ## Why model on the folder backend
//!
//! The folder backend ([`super::filesystem::FilesystemVaultWriter`]) is
...
//! [`VaultBackend::current_head`](crate::backend::VaultBackend::current_head)
...
//! pin and delegates to [`commit`](crate::backend::VaultBackend::commit).
...
/// Every byte the vault holds, behind one lock. Committed bytes are
/// the in-RAM analogue of the folder backend's on-disk directory; the
/// pending buffer folds into them on [`commit`](VaultBackend::commit).
#[derive(Default)]
struct State {
    /// Committed entity bytes keyed by normalised vault-relative path
    ...
    /// [`discard_pending`](VaultBackend::discard_pending).
    ...
    /// Per-vault `.memstead/config.json` bytes once written. `None`
    /// until the create path writes the config.
    config: Option<Vec<u8>>,
}

/// Writable vault backend whose entire state lives in memory. See the
...
impl InMemoryBackend {
    /// Build an empty in-memory vault. Nothing is provisioned on disk;
    /// the vault exists only for as long as this backend is held.
    pub fn new() -> Self {
        Self {
...
impl VaultBackend for InMemoryBackend {
    ...
    fn write_vault_config(&self, bytes: &[u8]) -> Result<(), BackendError> {
        ...
        // impl returns `Sealed`), the in-memory vault stores the config
...
mod tests {
    ...
    fn rejects_path_traversal_absolute_and_empty() {
        ...
        // `normalise_rel_path`, surfaced as VaultWriter path errors.
    ...
    fn vault_config_round_trips() {
        let b = InMemoryBackend::new();
        assert_eq!(b.read_vault_config().unwrap(), None);
        b.write_vault_config(b"{\"schema\":\"default@1.0.0\"}").unwrap();
        assert_eq!(
            b.read_vault_config().unwrap(),
