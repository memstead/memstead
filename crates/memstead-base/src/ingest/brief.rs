...
/// `PROCESS_MEM_SCHEMA`.
pub const PROCESS_MEM_SCHEMA: &str = "ingest@0.2.0";
...
mod tests {
    ...
    fn renders_situation_with_present_process_mem() {
        ...
        assert!(out.contains("A paired process mem `ingest/macos` (schema `ingest@0.2.0`) carries destination-quality debt"));
        assert!(
    ...
    fn situation_process_mem_branches() {
        ...
        assert!(out.contains("memstead mem init os --org-path ingest --schema ingest@0.2.0"));
    }
...
- **ingest/macos** — schema: `ingest@0.2.0`. Inspect via `memstead_overview` / `memstead_search mem=macos`.
\n";
