...
//! from each writable vault's `readVaults` field, instantiates each
...
use memstead_base::{
    BootError, Engine, FileWorkspaceStore, Mount, VaultBackend, WorkspaceStoreAdapter,
    detect_layout,
};

fn hydrate_read_vaults(
    writable_mounts: &[(Mount, Box<dyn VaultBackend>)],
    writable_names: &std::collections::HashSet<String>,
) -> Result<Vec<(Mount, Box<dyn VaultBackend>)>, BootError> {
    let cache_dir = crate::vault_cache::vault_cache_dir();
    let mut seen: std::collections::HashSet<String> = std::collections::HashSet::new();
    let mut extras: Vec<(Mount, Box<dyn VaultBackend>)> = Vec::new();
    for (_, backend) in writable_mounts {
        ...
        let config = match memstead_schema::config::parse_vault_config(&value) {
            Ok(c) => c,
            Err(_) => continue,
        };
        for (vault_name, spec) in &config.read_vaults {
            if writable_names.contains(vault_name) {
                continue;
            }
            if !seen.insert(vault_name.clone()) {
                continue;
            }
            ...
            // cross-vault validator about every RO mount's schema, so
            // `memstead relate` against an RO-mounted entity saw the engine
            // default and refused with `CROSS_VAULT_EDGE_NOT_DECLARED`
            ...
            let archive_schema = crate::vault_cache::read_published_config(&archive_path)
                .map(|cfg| cfg.schema)
                .unwrap_or_else(|_| {
...
pub fn engine_from_workspace_root(workspace_root: &Path) -> Result<Engine, BootError> {
    let workspace = match detect_layout(workspace_root) {
        ...
        // boots through this pro entry, so the unified lone-vault experience
    ...
    // Read-vault hydration. For each writable mount, read its
    // `__MEMSTEAD:vault.config.json` and attach each `readVaults` entry as
    // a read-only `ArchiveBackend` pointing at the global vault cache.
    let extra_read_mounts = hydrate_read_vaults(&mounts, &writable_names)?;
    ...
    let ref_schemas =
        match crate::vault_repo_schemas::GitBranchSchemaSource::for_workspace(workspace_root)
            .read_schemas()
        {
            Ok(schemas) => schemas,
            // Best-effort overlay: a stub/invalid `vault-repo/.git` or a
...
mod tests {
    ...
    /// into the engine's resolution catalogue at boot — so a vault can
...
write_rules: []
"#;
        crate::vault_repo_schemas::GitBranchSchemaSource::for_workspace(root)
            .write_schema(
                "refsrc",
                "0.1.0",
                &[
