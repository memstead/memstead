...
```
references becomes (mem prefix plus title slug). Hedge: `--type`/`--title`/`--section KEY=TEXT` is my best reading of the CLI shape (the update command uses `--section-unset KEY` and `--from <file.json>`); confirm with `memstead create --help`, and substitute the required section keys `type concept` reported.
...
```
`definition` and `explanation` are the sections every verified concept write on default@1.3.0 supplies; the id becomes.
