...
Verify every checkable claim in both answers against the engine's Rust source under `<source>/crates` and, where a claim is about behaviour, by running the release binary `<memstead>` in a scratch workspace of your own under `<run>/grader-Q2h` (create it with `<memstead> init --name probe --schema default@1.3.0 <run>/grader-Q2h/probe` or `<memstead> quickstart <run>/grader-Q2h/probe`; never touch any other workspace or repository).
