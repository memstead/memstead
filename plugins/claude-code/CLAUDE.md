...
This plugin provides MCP tools (prefix: `memstead_`) and slash commands for graph interaction.
...
The engine self-defends against a sibling instance advancing a mem under your feet (parallel terminal, forked subagent, another agent session): a `MEM_RELOADED` marker in a tool response means the affected mem was reloaded, and a stale `expected_hash` trips `HASH_MISMATCH`. When either appears, anything you currently have *in your conversation context* about entities of that mem is suspect — re-read via `memstead_entity` before answering questions that depend on prior content, and refetch before a follow-up `memstead_update`. No hook pre-announces drift per turn: the engine's own markers are the signal.
