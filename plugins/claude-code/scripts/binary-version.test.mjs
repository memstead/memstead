...
test('isAtLeast implements semver >=', () => {
  assert.ok(isAtLeast({ major: 0, minor: 2, patch: 0 }, ANCHORS_MIN));
  assert.ok(isAtLeast({ major: 0, minor: 2, patch: 5 }, ANCHORS_MIN));
  assert.ok(isAtLeast({ major: 1, minor: 0, patch: 0 }, ANCHORS_MIN));
  assert.ok(!isAtLeast({ major: 0, minor: 1, patch: 9 }, ANCHORS_MIN));
...
test('gate: capable only when a recorded version >= threshold', () => {
  const root = ws();
  recordBinaryVersion(root, { run: () => ({ status: 0, stdout: 'memstead 0.2.0' }) });
...
test('gate FAILS CLOSED for a below-threshold recorded version', () => {
  ...
  writeFileSync(join(root, '.memstead.cache/plugin/binary-version.json'), JSON.stringify({ version: '0.1.0' }));
  const g = anchorsGate(root);
  assert.equal(g.capable, false);
