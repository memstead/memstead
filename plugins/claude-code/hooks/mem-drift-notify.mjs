...
// when one or more writable mems advanced since the last prompt in
// this session. Tracks per-session last-seen-HEAD per mem in
// `<workspace_root>/.memstead.cache/drift/last-seen-heads-<session_id>.json`
// and emits a single `<system-reminder>` block listing drifted mems
// and their changed entity ids.
//
// HEAD enumeration goes through the engine via
// `memstead_health { include_config: true }`; per-mem entity deltas via
// `memstead_changes_since`. The plugin no longer reads `mem-repo/.git/`
// directly. Fails open: any internal error writes a one-line
// `[mem-drift-notify]` diagnostic to stderr and exits 0 with empty
...
import { sanitizeSessionId, runDriftNotify } from './mem-drift-notify-utils.mjs';
...
async function main() {
  ...
  if (result.status === 'probe-failed') {
    process.stderr.write(`[mem-drift-notify] ${result.message}\n`);
    return;
  }
...
main().catch((err) => {
  process.stderr.write(`[mem-drift-notify] unexpected error: ${err.message}\n`);
  process.exit(0);
});
