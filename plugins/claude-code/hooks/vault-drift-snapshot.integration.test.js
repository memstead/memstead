// End-to-end integration tests for vault-drift-snapshot.mjs.
...
// (vault-drift-notify.mjs) only flags HEAD advances the agent did not
// author.
//
// MCP layer is mocked via `withEngineFn` (mirrors auto-commit's
// pattern) — the mock reads from a real `git init`'d vault-repo to
...
// no-direct-git (engine-owns-vault-repo) rule.
...
import { mockEngine as notifyMockEngine } from './vault-drift-notify.integration.test.js';
  ...
  return async (_cmd, _timeout, fn) => {
    const client = {
      async callTool(name, _args) {
        ...
        const refs = parseRefList(refsResult.stdout || '').filter((r) => isTrackedVault(r.name));
        const writable_vaults = refs.map((r) => r.name);
        const vaults = refs.map((r) => ({
          name: r.name,
          vcs: { gitdir: join(vaultRepo, '.git'), worktree: vaultRepo, head: r.sha },
        }));
        return { writable_vaults, vaults };
      },
    };
    return fn(client);
  };
...
async function runSnapshot({ workspaceRoot, sessionId, vaultRepo }) {
...
async function runNotify({ workspaceRoot, sessionId, vaultRepo }) {
  ...
  return runDriftNotify({
    ...
    sessionId,
    engineCommand: { cmd: 'true', args: [], cwd: workspaceRoot },
    withEngineFn: notifyMockEngine(vaultRepo),
  });
}
...
describe('snapshot: turn with own mutations is followed by a silent next prompt', () => {
  ...
  before(async () => {
    ws = setupWorkspace();
    commitOnBranch(ws.vaultRepo, 'memstead/engine', { 'a.md': 'v1\n' }, 'eng v1');
    ...
    await runSnapshot({ workspaceRoot: ws.root, sessionId, vaultRepo: ws.vaultRepo });
  });
...
describe('snapshot: mixed turn (own + sibling) flags only the sibling vault', () => {
  let ws;
  const sessionId = 'sess-mixed';
  before(async () => {
    ws = setupWorkspace();
    commitOnBranch(ws.vaultRepo, 'memstead/engine', { 'a.md': 'v1\n' }, 'eng v1');
    commitOnBranch(ws.vaultRepo, 'memstead/macos', { 'm.md': 'v1\n' }, 'mac v1');
    await runNotify({ workspaceRoot: ws.root, sessionId, vaultRepo: ws.vaultRepo });
    commitOnBranch(ws.vaultRepo, 'memstead/engine', { 'a.md': 'v2\n' }, 'agent in eng');
    await runSnapshot({ workspaceRoot: ws.root, sessionId, vaultRepo: ws.vaultRepo });
    commitOnBranch(ws.vaultRepo, 'memstead/macos', { 'm.md': 'v2\n' }, 'sibling in mac');
  });
  after(() => rmSync(ws.root, { recursive: true, force: true }));
  ...
  it('reports only memstead/macos on the next UserPromptSubmit', async () => {
    ...
    assert.match(r.reminder, /Vault `memstead\/macos`/);
    assert.doesNotMatch(r.reminder, /Vault `memstead\/engine`/, 'own mutation must not appear');
  });
});
...
describe('snapshot: first-ever fire on a session with no prior state writes silently', () => {
  ...
  it('writes the state file silently and exits cleanly', async () => {
    const r = await runSnapshot({ workspaceRoot: ws.root, sessionId, vaultRepo: ws.vaultRepo });
...
describe('snapshot: branch filter excludes main and __* refs', () => {
  ...
  before(() => {
    ws = setupWorkspace();
    commitOnBranch(ws.vaultRepo, 'memstead/engine', { 'a.md': 'v1\n' }, 'v1');
    commitOnBranch(ws.vaultRepo, '__SYSTEM', { 'sys.md': 'v1\n' }, 'sys v1');
  });
  after(() => rmSync(ws.root, { recursive: true, force: true }));

  it('records only writable vault branches in the state file', async () => {
    const r = await runSnapshot({ workspaceRoot: ws.root, sessionId, vaultRepo: ws.vaultRepo });
    assert.strictEqual(r.status, 'snapshotted');
    const state = readState(ws.root, sessionId);
    assert.ok(state['memstead/engine']);
