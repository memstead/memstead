...
describe('checkBashCommand', () => {
  const vaultDir = 'specs';

  // Blocked: write operations on entity files
  it('blocks echo redirect to entity file', () => {
    const result = checkBashCommand('echo test > specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'block');
  });

  it('blocks sed -i on entity file', () => {
    const result = checkBashCommand('sed -i "" s/foo/bar/ specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'block');
  });

  it('blocks cp to entity file', () => {
    const result = checkBashCommand('cp /dev/null specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'block');
  });

  it('blocks mv of entity file', () => {
    const result = checkBashCommand('mv specs/test-core/spec-entity.md specs/test-core/renamed.md', vaultDir);
    assert.equal(result.action, 'block');
  });

  it('blocks rm of entity file', () => {
    const result = checkBashCommand('rm specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'block');
  });

  it('blocks git checkout on entity file', () => {
    const result = checkBashCommand('git checkout -- specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'block');
  });

  it('blocks git restore on entity file', () => {
    const result = checkBashCommand('git restore specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'block');
  });

  it('blocks tee to entity file', () => {
    const result = checkBashCommand('echo test | tee specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'block');
  });

  // Allowed: read operations on entity files
  it('allows cat of entity file', () => {
    const result = checkBashCommand('cat specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'allow');
  });

  it('allows cat piped to head', () => {
    const result = checkBashCommand('cat specs/test-core/spec-entity.md | head -5', vaultDir);
    assert.equal(result.action, 'allow');
  });

  it('allows git log on entity file', () => {
    const result = checkBashCommand('git log --oneline specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'allow');
  });

  it('allows git diff on entity file', () => {
    const result = checkBashCommand('git diff specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'allow');
  });

  it('allows wc on entity file', () => {
    const result = checkBashCommand('wc -l specs/test-core/spec-entity.md', vaultDir);
    assert.equal(result.action, 'allow');
  });

  // Allowed: write operations on non-entity files
  it('allows echo redirect to non-entity file', () => {
    const result = checkBashCommand('echo test > packages/core/lib/store.js', vaultDir);
    assert.equal(result.action, 'allow');
  });

  it('allows sed -i on non-entity file', () => {
    const result = checkBashCommand('sed -i "" s/foo/bar/ packages/core/lib/store.js', vaultDir);
    assert.equal(result.action, 'allow');
  });

  // Allowed: write operations on non-entity-named .md in specs/
  it('allows write to README.md in specs/', () => {
    const result = checkBashCommand('echo test > specs/README.md', vaultDir);
    assert.equal(result.action, 'allow');
  });

  it('allows write to UPPERCASE.md in specs/', () => {
    const result = checkBashCommand('echo test > specs/domain/STUPID_USER_FILE.md', vaultDir);
    assert.equal(result.action, 'allow');
  });

  it('allows write to underscore .md in specs/', () => {
    const result = checkBashCommand('echo test > specs/domain/my_notes.md', vaultDir);
    assert.equal(result.action, 'allow');
  });

  // Edge cases
  it('allows empty command', () => {
    assert.equal(checkBashCommand('', vaultDir).action, 'allow');
    assert.equal(checkBashCommand(null, vaultDir).action, 'allow');
    assert.equal(checkBashCommand(undefined, vaultDir).action, 'allow');
  });

  it('truncates long commands in reason', () => {
    const longCmd = 'echo ' + 'x'.repeat(200) + ' > specs/domain/entity.md';
    const result = checkBashCommand(longCmd, vaultDir);
    assert.equal(result.action, 'block');
    assert.ok(result.reason.endsWith('...'));
  });

  // Multi-vault: reasoning directory protection
  it('blocks write to entity file in reasoning/ vault', () => {
    const result = checkBashCommand('echo test > reasoning/architecture/my-memo.md', 'reasoning');
    assert.equal(result.action, 'block');
  });

  it('allows read of entity file in reasoning/ vault', () => {
    const result = checkBashCommand('cat reasoning/architecture/my-memo.md', 'reasoning');
    assert.equal(result.action, 'allow');
  });

  it('does not match reasoning/ when checking specs vault', () => {
