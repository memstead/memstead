...
This is the folder-workspace onboarding flow — the binaries are `memstead` (CLI) plus `memstead-mcp` (MCP server). It sets up no mem-repo, no multi-mem, no git history layer — that shape comes from `memstead mem-repo init` and is not reached from this skill.
