...
## Scope: code-bound mems only

Reconcile syncs a mem **against a code source** — it applies to code-bound mems whose entities describe a codebase (specs, contracts, decisions realized in source). A **knowledge-only mem** (no code source — research, worldbuilding, personal knowledge) has nothing to reconcile *from*: refresh it by ingesting new material (`/ingest`) or re-interviewing (`/interview`), not by reconcile. If the target mem has no code source, say so and stop — do not attempt a code-source scan against it.

## Phase 1: Discover mems and sources

1. **Discover mems from the engine — not from `.mcp.json`.** The engine is the source of truth for which mems are mounted and where their files live; it may be launched any way (e.g. `cd graph && exec memstead-mcp`, with no `--mem` arguments at all), so parsing the MCP launch command is unreliable. Call `memstead_overview` (the `## Mems` list) or `memstead_health { include_config: true }` and take the mem set from there.

2. **Determine where each mem's entity files live (backend-dependent).** This decides whether the source scan needs to exclude anything. Resolve each mem's gitdir via `memstead_health { include_config: true }` → `mems[].vcs.gitdir`.
   - **git-branch backend** — the mem is a branch inside a nested `mem-repo/` gitdir, itself gitignored by the outer project repo. Its entity `.md` files never appear in the outer `git status`. **Nothing to exclude.**
   - **folder backend** — the mem is a directory of `.md` files in the outer working tree. Its entity files *would* show up in the source scan and must be excluded.

3. **Enumerate source repos.** A source repo is any directory containing a `.git/` folder. Today's layout is one outer project repo with the git-branch `mem-repo/` nested inside it and gitignored; stay correct for future multi-repo or folder-backend workspaces.

## Phase 2: Find what changed since the last sync (the per-source cursor)

Reconcile updates only what changed **since the last time it synced this source** — never the whole tree every run. That "last time" is a **per-source sync cursor**: the source commit SHA reconcile last synced from, recorded per mem-source pair.

1. **Read the cursor.** Look for `.memstead/reconcile-cursors.json` — reconcile's own bookkeeping (not graph state, not engine state). Shape: `{ "<mem>:<repo-path>": "<last-synced-commit-sha>", ... }`. A missing file or a missing pair means this is a first sync for that source.

2. **Compute the changed set per source repo.** Always exclude reconcile's own bookkeeping and config from the source scan — add `':!.memstead/**'`, plus `':!<mem-dir>/**/*.md'` for any *folder-backend* mem under that repo. **Quote every pathspec in single quotes** so the shell doesn't expand `!`.
   - **Have a cursor:** the changed source since last sync is
     ```bash
     git diff --stat <cursor-sha>..HEAD -- ':!.memstead/**' ':!<memDir>/**/*.md'
     git diff        <cursor-sha>..HEAD -- ':!.memstead/**' ':!<memDir>/**/*.md'
     ```
...
These MCP mutations are committed per-mutation by the engine to each mem's gitdir — reconcile does not commit them.

## Phase 4: Advance the cursor + summarize

1. **Advance the cursor.** Write `.memstead/reconcile-cursors.json`, setting each synced `<mem>:<repo-path>` to that source's current `git rev-parse HEAD`. This file is reconcile's only on-disk write outside the graph, and it is **not committed** — it is local bookkeeping; keep it out of your code commits (it is excluded from the source scan above, and belongs in `.gitignore`).
...
## Creating and removing mems

Mem lifecycle is a workspace-policy operation, not part of the reconcile flow. The engine exposes two MCP tools — `memstead_mem_create` and `memstead_mem_delete` — gated by `[[mem_management.create]]` / `[[mem_management.delete]]` rules in `.memstead/workspace.toml` (each rule carries a glob `pattern` over the composed `<path>/<name>` candidate, plus per-rule `schemas[]` on the create side). When neither array is present or both are empty, both tools return `MEM_PATH_NOT_ALLOWED` for every input — the default posture is conservative, and the workspace operator opts in explicitly per namespace.

Discover the live lifecycle policy via `memstead_overview`'s `## Lifecycle Namespaces` section (each rule lists `pattern`, `actions`, and the create-side `schemas`). Each writable mem's `origin` (`explicit`, `discovered`, or `runtime_created`) ships under `memstead_health { include_config: true }`. Reconcile itself never calls these lifecycle tools — mem creation and deletion are intentional acts by a dedicated skill or the user.
...
- Spec changes are committed **per-mutation** by the engine to each mem's own gitdir (default `<mem>/.git/`, or a shared-gitdir path when configured — resolve the canonical path via `memstead_health { include_config: true }` and read `mems[].vcs.gitdir`) — not debounced, not batched. Entity `.md` files in the *outer* workspace repo stay dirty until the outer-repo auto-commit lands or `/commit` runs manually. Reconcile does not stage or commit them.
