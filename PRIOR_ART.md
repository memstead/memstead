...
The category splits along one axis. Most of it **derives** memory: an extraction pipeline distils conversations or documents into a retrieval store, and the agent recalls from it. Memstead sits at the **authored** end: agents and humans deliberately write typed entities into a schema-validated graph, stored as markdown in a git repository the user owns. Both approaches are legitimate; they optimise different things. Derived memory optimises recall per token. An authored mem is an agent-curated model of a subject — deliberately selected and maintained, reviewable, diffable, and portable as a versioned package. Notably, the substrate bet is no longer unique: three of the six systems below store memory as markdown files, and one of those keeps them in git. Nor is write-time schema rejection — IWE ships it. What stays distinctive is the combination above the substrate: a controlled, typed relationship vocabulary with endpoint constraints, schemas bound to entity types and packaged as versioned artifacts, and structured commit provenance that travels inside the published package (the sealed `.mem` carries a provenance record for every entity, with the authoring rationale where one was recorded, readable offline).
...
[IWE](https://github.com/iwe-org/iwe) (Apache-2.0, Rust) is the closest neighbour overall, and it arrived at strikingly similar architecture independently: a shared core library (`liwe`) drives a CLI, an LSP server, and an MCP server over plain markdown files on disk. It positions itself as a note-graph tool, not agent memory — but its 2026 document-schema dialect puts it squarely in this comparison: frontmatter validated as literal JSON Schema draft 2020-12, the body validated as an ordered section tree (`minContains`/`maxContains`, header `const`/`enum`/`pattern` constraints, token budgets), schemas bound to documents by path glob and composed `allOf`-style. Enforcement is write-time and hard — its MCP docs call schema validation "the only hard reject" — and violations carry a breadcrumb, hint, schema pointer, and keyword: machine-repairable refusals in the same spirit as Memstead's recovery payloads.

What it is genuinely good at: markdown knowledge-base refactoring (block-reference transclusion, extract/inline operations, normalization) with the same structure available to editors via LSP and to agents via MCP. Two of its agent-surface designs deserve plain acknowledgement: write responses piggy-back advisory graph-health warnings (orphans, dangling links), de-duplicated per session so the agent sees standing issues without a separate health call; and bulk mutations require explicit per-operation `expect` guards, refusing calls that omit them. It independently validates three of Memstead's bets — the markdown substrate, one engine behind CLI/LSP/MCP surfaces, and rejection-on-write with structured errors.

How Memstead differs: the graph, git, and packaging. IWE's edge model has exactly two kinds, both untyped — inclusion and reference; there is no relationship vocabulary, no endpoint constraints, no cardinality, no acyclicity, and schemas bind by file path rather than by entity type, so "a Decision may only supersede a Decision" is inexpressible there. Git is the user's business — the engine never commits, so there is no structured provenance. And schemas are loose workspace files: no versioning, no packaging, no registry, whereas a mem travels as a sealed archive with its pinned, versioned schema embedded.

<!-- Sources: https://github.com/iwe-org/iwe (README: liwe core + iwe/iwes/iwec binaries, licence), docs/document-schema.md (dialect, JSON Schema 2020-12 frontmatter, section-tree keywords, glob binding), docs/mcp.md (tool surface, "schema validation remains the only hard reject", advisory warnings, expect guards), docs/spec.md (§5.2: two untyped edge kinds), docs/comparison.md (shared-core positioning). Retrieved 2026-08-06. -->
