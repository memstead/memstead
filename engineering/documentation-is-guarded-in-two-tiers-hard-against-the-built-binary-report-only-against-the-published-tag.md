...
last_modified: 2026-08-23T15:50:35Z
status: accepted
...
First catch at landing: `docs/proof/reconstruction/README.md` documented `memstead stats` (the command is `status`). Findings on the present tree that the repair plan owns: the retired slash prefix in the local-LLM notes folder, "8 crates" in the engine chapter, three rows missing from the constitution's structure table. The private prose set was clean against the built binary once the checker learned that a scheme-prefixed link target points outside the tree.
