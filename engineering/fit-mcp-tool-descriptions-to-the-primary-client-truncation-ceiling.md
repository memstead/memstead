...
last_modified: 2026-09-08T21:08:44Z
status: accepted
...
## Consequences
- Descriptions are budgeted: every wording change must keep each tool inside the meta-suite's word band and under the byte ceiling with its backtick refs lintable — enforced by `tool_surface.rs` (corrected 2026-09-08: the one build's suite since 2026-09-05).
...
- The guard binds the one server (corrected 2026-09-08: it bound both flavours until the one-flavour fold of 2026-09-05). Corrected 2026-08-30: this bullet used to say the lean filesystem surface carried unlinted descriptions the ceiling never checked. `tool_surface.rs`'s `descriptions()` harvester now enumerates the `full` and `lean` routers alike, so the byte ceiling, the backtick-reference lint and the leading-verb rule covered the lean build's descriptions too. Corrected 2026-09-08: the lean router left on 2026-09-05; the harvester walks the one live router.
