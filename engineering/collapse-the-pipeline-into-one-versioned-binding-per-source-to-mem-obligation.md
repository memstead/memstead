...
**Superseded 2026-07-18; corrected 2026-08-30.** This record describes the **v1** binding, which is no longer the on-disk or in-memory shape, and it must not be read as current guidance. “Consolidate the pipeline into one record per pipeline with inline sources” (accepted, 2026-07-18) folded the standalone medium and facet records into the binding: the live format is **v2** (`BINDING_VERSION = 2` in `crates/memstead-base/src/binding.rs`), one record at `.memstead/projections/<mem>/<name>.json` whose inline `sources[]` carry both halves v1 kept in separate records. The “format is frozen at E2's completion” statement above therefore no longer holds: v2 was a deliberate, non-backwards-compatible break, and the engine reads only v2 (a v1 or gen-2 store refuses at load and at boot with a typed error naming `memstead projection migrate`). Everything this record says about `version: 1`, about *source facets* as a joinable record kind, and about the frozen format is history. What survived into v2, and is still live: the binding as the unit of obligation, the `operations { build, sync, verify }` block, `hash(D)`, the per-medium capability matrix, the `memstead projection {brief,init,migrate,advance,enable}` CLI tree, and `memstead status`.
