...
last_modified: 2026-09-21T11:30:08Z
status: accepted
...
A single generic `BOOT_FAILED` code was rejected as typed-in-name-only: the agent's next step differs per class (pin repair vs projection migrate vs storage residue), so the code must too. Fixing only the projection case (the literal backlog item) was rejected because the class recurs per boot-failure source — the 2026-08-06/07 schema-pin outage was a different member of the same class. Keeping the install-hint suppression for wrong-version pins was refined rather than kept: when the pinned version is absent but the name exists at other versions, the honest hint is version repair, not silence.
