...
- A graph source declaring `entity-load-bearing` gets the zero-false-positive drift metric mechanically: notes-only edits keep dependents resolving, load-bearing edits drift them. The standalone `verify-anchors` inherits it through the shared observation site.
