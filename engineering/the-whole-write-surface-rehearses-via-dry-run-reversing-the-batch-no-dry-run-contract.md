...
last_modified: 2026-09-08T21:11:14Z
status: accepted
...
A separate `memstead_validate` tool — rejected: tool-count policy, and a separate tool invites drift from the real verbs; the flag on the verb IS the shared-code guarantee. Keeping the batch no-dry-run contract — rejected as policy outliving purpose (this decision is the recorded reversal). Implementing dry-run on the filesystem flavour — rejected: it was the deliberately lean surface; a typed refusal is honest and cheap. Corrected 2026-09-08: that surface left with the one-flavour fold of 2026-09-05; the folder workspace is served by the one engine. Rehearse-then-commit tokens (validate once, apply later without re-validating) — rejected: the mem can change between calls; re-validation on the real call is the only honest semantics.
