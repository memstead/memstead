...
## Context
`memstead init` bootstraps a filesystem-mem before any `.memstead/schemas/` directory exists, and `memstead schema install` only operates *inside* an already-initialised workspace (the write side of [[engine--schema-storage-source-seam]]). Corrected 2026-09-08: on the lean build there was additionally no `mem set-schema` to re-pin later; since the one-flavour fold of 2026-09-05 the verb ships in every binary. So for any pin outside the [[engine--built-in-schema-catalogue]], the only path to a custom-schema mem is init-with-pin followed by install — which is impossible if init refuses an unresolved pin. The sibling `create_mem` / `memstead_mem_create` path ([[engine--mem-lifecycle-operations]]) runs against an already-existing workspace where install-before-pin is always possible, so it can afford to resolve the pin eagerly and refuse.

## Consequences
- The folder-workspace custom-schema authoring flow (corrected 2026-09-08: called the lean flow until 2026-09-05) works: `memstead schema new` -> `schema validate` -> `memstead init --schema <new>@<v>` -> `schema install`, exactly the follow-up the [[engine--cli-cold-start-quickstart-and-schema-scaffold-surface]] documents.
...
- **Refuse eagerly (as `create_mem` does)** — resolve the pin at init and reject an unresolved one: rejected; it makes the folder-workspace custom-schema flow (the lean flow until 2026-09-05; corrected 2026-09-08) impossible, because the schema can only be installed after the workspace exists.
